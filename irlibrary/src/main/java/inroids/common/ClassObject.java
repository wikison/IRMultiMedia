...
public abstract class ClassObject {
    private static final String TAG = "IRLibrary";
    ...
    public static int getGravity(JSONObject objT, String strKey) {
        ...
        try {
            ...
            if (System.isNotNullString(strAlignment)) {
    ...
    public static boolean setFakeBoldText(TextView txtT, JSONObject objT, String sKey) {
        ...
        if (intT == 1)
            txtT.getPaint().setFakeBoldText(true);
        return intT == 1;
    }
    ...
    public static boolean setTextSkew(TextView txtT, JSONObject objT, String sKey) {
        int intT = ClassObject.getInt(objT, sKey);
        if (intT == 1)
            txtT.getPaint().setTextSkewX(-0.5f);
        return intT == 1;
    }
     ...
     *            a JSON Object.
     * @param sBgColor
     *            a key string with bgColor
     * @param sRadian
     *            a key string with Radian
     */
            ...
            if (intBgColor < 0) {
                ...
                if (intRadian > 0)
                    gd.setCornerRadii(new float[]{
                        intRadian, intRadian, intRadian, intRadian, intRadian, intRadian, intRadian, intRadian
                    });
            }
        } catch (Exception e) {
    ...
    public static int getInt(JSONObject objT, String sKey) {
        try {
            ...
            if (!objT.isNull(sKey)) {
                ...
                if (System.isNotNullString(strT)) {
    ...
    public static int getColor(JSONObject objT, String sKey) {
        try {
            if (!objT.isNull(sKey) && objT.getString(sKey).length() > 0) {
                ...
                if (System.isNotNullString(sColor)) {
                    return Convert.stringToColor(objT.getString(sKey));
                }
            }
        } catch (JSONException e) {
    ...
    public static boolean getBoolean(JSONObject objT, String sKey) {
        try {
            if (!objT.isNull(sKey) && objT.getString(sKey).length() > 0 && objT.getString(sKey).length() == 1) {
                int i = objT.getInt(sKey);
                return i != 0;
            } else if (!objT.isNull(sKey) && objT.getString(sKey).length() > 0 && objT.getString(sKey).length() > 1) {
                String s = objT.getString(sKey);
    ...
    public static JSONArray getJSONArray(JSONObject objT, String sKey) {
        try {
            if (!objT.isNull(sKey) && objT.getString(sKey).length() > 0) {
                return objT.getJSONArray(sKey);
            }
        } catch (JSONException e) {
            MyLog.e(TAG, "ClassObject.getJSONArray:" + e.toString() + " key:" + sKey);
        }
        return null;
    }
}
